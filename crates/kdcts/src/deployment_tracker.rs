...
/// Unique deployment key: (image_name, node_uuid)
...
/// Keyed by (image_name, node_uuid) — the same image can be deployed to multiple nodes.
...
pub async fn record_deployment(
    ...
    let key = (image_name.to_string(), node_uuid.to_string());
    ...
    info!(
        ...
        image_name, node_uuid,
        guard.get(&(image_name.to_string(), node_uuid.to_string())).map_or(0, |d| d.server_ports.len())
    );
}
...
/// Remove all deployments for a given node UUID (on disconnect).
pub async fn remove_by_node(
    ...
    let keys: Vec<DeploymentKey> = guard
        .iter()
        .filter(|(_, d)| d.node_uuid == node_uuid)
        .map(|(k, _)| k.clone())
        .collect();
